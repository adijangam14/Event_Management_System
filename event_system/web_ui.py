...
def api_event_details(event_id):
    ...
    if event_details:
        event_id, event_name, event_date, event_time, venue, total_slots = event_details
        return jsonify({
            'event_id': event_id,
            'event_name': event_name,
            'event_date': event_date.strftime('%Y-%m-%d') if isinstance(event_date, (datetime.date, datetime.datetime)) else str(event_date),
...
# db.init_pool()  <-- Moved to __main__.py to avoid side effects on import
...
def inject_web_theme():
    return dict(web_theme=config.WEB_THEME)
...
def students_page():
    ...
    if form.validate_on_submit():
        ...
        result = students.add_student(session.get('role'), student_id, name, email, course, year)
    ...
    # The authorization check is now handled in the business logic,
    # but we might still want to only show the form to admins.
    # For now, we pass the role to the template and let it decide.
...
def edit_student(student_id):
    ...
    if request.method == 'GET':
        ...
        form.name.data = student[1]
        form.email.data = student[2]
        form.course.data = student[3]
        form.year.data = student[4]
    ...
    if form.validate_on_submit():
        ...
        result = students.update_student(session.get('role'), student_id, name, email, course, year)
...
def delete_student(student_id):
    ...
    result = students.delete_student(session.get('role'), student_id)
    if "Success" in result:
        flash(result, 'success')
    else:
        flash(result, 'danger')
...
def events_page():
    ...
    if form.validate_on_submit():
        ...
        # Pass the user's role to the business logic function
        result = events.create_event(session.get('role'), event_name, event_date, event_time, venue, total_slots)
...
def registrations_page():
    ...
    if form.validate_on_submit():
        ...
        result = registrations.register_student_for_event(session.get('role'), event_id, student_id)
...
def cancel_registration():
    ...
    if form.validate_on_submit():
        ...
        result = registrations.cancel_registration(session.get('role'), event_id, student_id)
...
def attendance_page():
    if 'username' not in session or session.get('role') not in ['admin', 'volunteer']:
        return redirect(url_for('login'))
...
def reports_page():
    ...
    if selected_event_id:
        ...
        if stats:
            # Generate the chart and get the path
            chart_path = reports.generate_attendance_chart(selected_event_id)
...
def emails_page():
    ...
    if request.method == 'GET' and all_events:
        # We don't set data here anymore because the JS in the template
        # will automatically fill it based on the default selected event and template.
        pass
