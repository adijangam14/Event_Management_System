...
def create_event_notification_email_body(event_name, event_date, event_time, event_location, custom_message=""):
    """
    Creates the body of an event notification email, sanitizing inputs to prevent header injection.
    """
    # Sanitize all inputs to remove newline characters
    safe_event_name = str(event_name).replace('\n', ' ').replace('\r', '')
    safe_event_date = str(event_date).replace('\n', ' ').replace('\r', '')
    safe_event_time = str(event_time).replace('\n', ' ').replace('\r', '')
    safe_event_location = str(event_location).replace('\n', ' ').replace('\r', '')
    safe_custom_message = str(custom_message).replace('\n', ' ').replace('\r', '')

    body = f"""
Dear Attendee,

This is a notification regarding the upcoming event: **{safe_event_name}**.

Date: {safe_event_date}
Time: {safe_event_time}
Location: {safe_event_location}
